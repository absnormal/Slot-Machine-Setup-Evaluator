        ...
        const GAS_URL = "https://script.google.com/macros/s/AKfycby1bfxQa5rGdVUrr_gXoZ3bBgin-c8bvVnohqPkdipicaG2m8vWxz9y7rubpgQ8avGbzg/exec";

        // AI API Key
        const apiKey = "";

        // === 共用輔助函式 ===
        const toPx = (val, max) => (val / 100) * max;
        ...
        const ptFileToBase64 = (file) => new Promise((resolve, reject) => {
        ...
        // === 特殊符號判定輔助函式 ===
        const isScatterSymbol = (sym) => sym && sym.toUpperCase().includes('SCATTER');
        const isCollectSymbol = (sym) => sym && sym.toUpperCase().includes('COLLECT');
        const isWildSymbol = (sym) => sym && sym.toUpperCase().includes('WILD');
        const isCashSymbol = (sym) => sym && sym.toUpperCase().startsWith('CASH');
        const getCashValue = (sym) => {
        ...
        function App() {
          ...
          `;
            ...
            const computeGridResults = useCallback((targetGrid, betAmount) => {
            ...
            }, [template, getSafeGrid]);

            useEffect(() => {
                const { results, error } = computeGridResults(panelGrid, betInput);
                setCalcResults(results);
                setCalculateError(error);
            }, [panelGrid, betInput, computeGridResults]);
            ...
            useEffect(() => {
                ...
                const { results, error } = computeGridResults(visionGrid, betInput);
                setVisionCalcResults(results);
                setVisionCalculateError(error);
            }, [visionGrid, betInput, computeGridResults]);

            const renderResultView = (calcData, calcErr, hoveredId, setHoveredId, showAll, setShowAll) => {
                return (
                    <div className="relative flex flex-col h-full lg:block w-full">
                        <div className="static lg:absolute lg:inset-0 flex flex-col w-full h-full">
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col flex-1 min-h-0">
                                ...
                                <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200 shrink-0">
                                    <div className="flex-1 w-full">
                                        ...
                                        <div className="relative">
                                            ...
                                            <input type="number" value={betInput} onChange={(e) => setBetInput(e.target.value)} className="w-full pl-10 pr-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-black text-lg text-indigo-700 shadow-sm transition-shadow" />
                                        </div>
                                    </div>
                                    <div className="px-4 py-2 bg-emerald-50 border border-emerald-200 text-emerald-700 font-bold rounded-lg flex items-center justify-center gap-2 shadow-sm pointer-events-none select-none shrink-0 self-end sm:self-auto h-[46px] mt-0 sm:mt-6 w-full sm:w-auto">
                ...
                <div className="min-h-screen bg-slate-50 text-slate-800 p-6 font-sans relative">
                    ...
                    <div className="max-w-7xl mx-auto space-y-6">
                        ...
                        <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                            ...
                            <button onClick={() => setShowSettingsModal(true)} className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-lg shadow-sm hover:bg-slate-50 transition-colors text-sm font-bold text-slate-700 shrink-0">
                            ...
                            {!isPhase2Minimized && (
                                <div className="p-6 pt-0 border-t border-slate-100 mt-4 bg-slate-50">
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                        ...
                                        {/* Phase 2 專屬結算 UI */}
                                        {renderResultView(calcResults, calculateError, hoveredLineId, setHoveredLineId, showAllLines, setShowAllLines)}
                                    </div>
                                </div>
                            )}
                        </div>
                                ...
                                <div className="p-6 pt-0 border-t border-slate-100 mt-4 bg-slate-50">
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                        ...
                                        {activeVisionImg && visionGrid ? (
                                            renderResultView(visionCalcResults, visionCalculateError, visionHoveredLineId, setVisionHoveredLineId, visionShowAllLines, setVisionShowAllLines)
                                        ) : (
                                            <div className="relative flex flex-col h-full lg:block w-full">
                                                <div className="static lg:absolute lg:inset-0 flex flex-col w-full h-full">
                                ...
                                <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                                    <div className="flex items-center gap-3">
                                        ...
                                        {isBackgroundSyncing ? (
                                            ...
                                            <button onClick={handleForceRefreshCloud} className="flex items-center gap-1 text-[10px] bg-slate-100 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 border border-slate-200 px-2 py-1 rounded-full font-bold transition-colors" title="強制繞過快取重新整理">
                                                <RefreshCw size={10} /> 重新整理
                                            </button>
                                        )}
                                    </div>
                                    <button onClick={() => setShowCloudModal(false)} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-200 rounded-full transition-colors"><X size={20} /></button>
                                </div>
                                                        ...
                                                        {t.creatorId === localUserId && (
                                                            deletingId === t.id ? (
                                                                <div className="flex gap-1 bg-rose-50 p-1 rounded-lg border border-rose-100">
                                                                    <button onClick={() => handleDeleteTemplate(t.id)} className="px-3 py-2 bg-rose-600 text-white text-xs font-bold rounded">刪除</button>
                        ...
                        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4" style={{ zIndex: 99999 }}>
                            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden animate-in fade-in zoom-in duration-200">
                                <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                                    ...
                                    <button onClick={() => setShowSettingsModal(false)} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-200 rounded-full transition-colors"><X size={20} /></button>
                                </div>
                                ...
                                <div className="p-4 border-t bg-slate-50 flex justify-end gap-3">
                                    <button onClick={() => {
                                        ...
                                        setTimeout(() => setTemplateMessage(''), 3000);
                                    }} className="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-lg shadow-md transition-colors">
                                        儲存並關閉
                                    </button>
                                </div>
                            </div>
